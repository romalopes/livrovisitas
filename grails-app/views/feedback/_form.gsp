<%@ page import="br.com.k2sistemas.livroVisitas.Feedback" %>
...
<div class="fieldcontain ${hasErrors(bean: feedbackInstance, field: 'user', 'error')} required">
	...
	<g:select id="user" name="user.id" from="${br.com.k2sistemas.livroVisitas.User.list()}" optionKey="id" required="" value="${feedbackInstance?.user?.id}" class="many-to-one"/>
</div>
