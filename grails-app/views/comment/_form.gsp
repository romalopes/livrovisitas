<%@ page import="br.com.k2sistemas.livroVisitas.Comment" %>
...
<div class="fieldcontain ${hasErrors(bean: commentInstance, field: 'user', 'error')} ">
	...
	<g:select id="user" name="user.id" from="${br.com.k2sistemas.livroVisitas.User.list()}" optionKey="id" value="${commentInstance?.user?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: commentInstance, field: 'feedback', 'error')} required">
	...
	<g:select id="feedback" name="feedback.id" from="${br.com.k2sistemas.livroVisitas.Feedback.list()}" optionKey="id" required="" value="${commentInstance?.feedback?.id}" class="many-to-one"/>
</div>
